...
/*
-Prevent 2 dots from being in the same spot
-Prevent dot from not moving
 */
...
const D = {
	...
	init: function (system, options) {
		...
		let initDot = function () {
			...
			let dotPosition = [(PS.random(D.GRID_WIDTH) - 1), (PS.random(D.GRID_HEIGHT) - 1)]

			PS.scale(dotPosition[0], dotPosition[1], PS.random(50) + 50);
	...
	touch: function (x, y, data, options) {
		...
		let updateDot = function (oldX, oldY) {
			...
			let dotPosition = [0,0];
			let newX = (PS.random(D.GRID_WIDTH) - 1);
			let newY = (PS.random(D.GRID_HEIGHT) - 1);
			...
			if (newX !== oldX || newY !== oldY) {
				...
				updateDot(oldX, oldY);
				return 0; // End function early to prevent multiple dots from being created.
			}
